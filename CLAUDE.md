...
- **Real data now lives in the DB**: one auth user (Elior, `elior015@gmail.com`, password now set via the recovery flow — see "Known gaps / TODO" below), one `household_members` row, one `connections` row for Max, 2 `accounts`, 56 real `transactions`. This is genuine household financial data from a live scrape, not a fixture.
...
- **M1 web app**: shadcn/ui + TanStack Query + Recharts installed; responsive nav shell (`(app)` route group — desktop sidebar / mobile bottom tabs) wraps Dashboard (`/`), Transactions, Budgets, and Sync Health pages. PWA manifest + svg icon added. Sync Health's "Sync now" button inserts a `sync_requests` row that `apps/agent/src/daemon.ts` (new polling loop, ~30s interval) picks up — the daemon must be running (`pnpm exec tsx src/daemon.ts`) for that button to do anything. **Password-reset flow is live**: `/login` has a "Forgot password?" form (`requestReset` → `resetPasswordForEmail` → `/auth/callback` → `/reset-password`, where `updateUser({ password })` sets the credential). `/auth/callback` is a **dual-flow** route handler (PKCE `?code=` exchange *and* implicit `#access_token=` fragment → inline-script POST → `setSession`) — see "Real bugs found by actually running this" for why both branches are needed. Elior's password was set through this flow and sign-in at `/login` is proven working.
- **Recovery-link helper (admin-API exception)**: `apps/agent/scripts/generate-recovery-link.ts` is a one-off that calls `admin.generateLink({ type: 'recovery' })` and opens the link directly, bypassing Supabase's free-tier built-in email sender (which is `429 over_email_send_rate_limit`-throttled — every `POST /recover` was returning 429). It does NOT mint a session and does NOT set a password; it only substitutes for the rate-limited email *delivery* channel, after which the real `/reset-password` UI sets the credential. Used once to unblock Elior's password set. The action_link is a one-time bearer token, so the script never prints it (pass `--print` only if `open` won't launch a browser).
...
- **Elior's auth account now has a password set, and sign-in at `/login` is proven working.** The credential was set by Elior through the real `/reset-password` UI (`updateUser({ password })`) after a recovery link was delivered — NOT by the assistant minting a session or setting a password via the admin API. The one admin-API exception used (`apps/agent/scripts/generate-recovery-link.ts`, `admin.generateLink({ type: 'recovery' })`) only substituted for the *email delivery* channel, which was `429 over_email_send_rate_limit`-throttled on the free tier; it neither mints a session nor sets a password. The M1 UI walkthrough (Dashboard / Transactions / Budgets / Sync Health) is now accessible in a browser at `http://localhost:3000` — sign in at `/login` and click through each page. The daemon must be running (`pnpm exec tsx src/daemon.ts` from `apps/agent/`) for Sync Health's "Sync now" button to do anything.
...
- **PKCE callback drops `generateLink` recovery tokens (implicit flow mismatch).** `admin.generateLink({ type: 'recovery' })` returns a link whose redemption goes through GoTrue `/verify` with `login_method: implicit` — it redirects to `/auth/callback` with `access_token` + `refresh_token` in the URL **fragment** (`#access_token=...`), not a `?code=` query param. The fragment is never sent to the server, so a Next.js route handler physically cannot read it. The original PKCE-only `/auth/callback` (which expected `?code=` and called `exchangeCodeForSession`) saw no code, dropped the fragment, and bounced to `/login` — no session, password never settable. Auth logs confirmed the `/verify` 303 carried `login_method: implicit`, which is the diagnostic that pinned it. Fixed by making `/auth/callback` a **dual-flow** route: the GET handler keeps the PKCE `?code=` branch but adds a no-`code` branch that returns HTML with an inline `<script>` reading `window.location.hash` and POSTing the tokens to a same-origin POST handler that calls `setSession` (cookies set on the response, origin checked to block cross-origin forced-login). **Lesson: admin-`generateLink` recovery links are implicit-flow, not PKCE. If you build a callback that only handles `?code=` exchange, `generateLink` links will dead-end at `/login` with no error — build the fragment branch too, or use the normal `resetPasswordForEmail` (PKCE) path when email delivery works.**
